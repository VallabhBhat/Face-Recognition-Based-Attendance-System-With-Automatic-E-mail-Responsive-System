...
wb.save('attendance.csv')'''

def output(filename, sheet,num, name, present):
    my_file = Path('attendance.csv'+filename+str(datetime.now().date())+'.xls');
    if my_file.is_file():
        rb = open_workbook('attendance.csv'+filename+str(datetime.now().date())+'.xls');
        book = copy(rb);
        sh = book.get_sheet(0)
        # file exists
    else:
    ...
    book.save('attendance.csv'+fullname)
    return fullname;
